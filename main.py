...
# Επιβολή SSL ανεξάρτητα από το URL string
...
def register():
    ...
    return render_template("register.html", form=form, current_user=current_user, current_year=datetime.now().year)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            login_user(user)
            return redirect(url_for("get_all_posts"))
    return render_template("login.html", form=form, current_user=current_user, current_year=datetime.now().year)
...
def get_all_posts():
    ...
    return render_template("index.html", all_posts=posts, current_user=current_user, current_year=datetime.now().year)
...
def add_new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for("get_all_posts"))
    return render_template("make-post.html", form=form, current_user=current_user, current_year=datetime.now().year)
...
def edit_post(post_id):
    ...
    return render_template("make-post.html", form=edit_form, is_edit=True, current_user=current_user, current_year=datetime.now().year)
